...
function currentSnapshot() {
  ...
  const summary = summarizeRows(filteredRows);
  const { groupCol, xCol, yCol } = getMapColumns();
  const bounds = getGlobalBounds(rowsForMapType(state.rawRows, mapType), xCol, yCol);
  return {
...
function getDisplayedMapConfigs(mapType = state.mapType, groupValue = state.groupValue) {
  const { groupCol } = getMapColumns(mapType);
  if (!state.rawRows.length) return [];
  if (groupValue === 'MERGE') {
    return [{ rows: rowsForMapType(state.rawRows, mapType), titleGroup: 'MERGE', isMerge: true, mapType }];
  }
  const groups = groupValue === 'ALL' ? uniqueValues(groupCol, state.rawRows) : [groupValue];
...
function buildMapWorksheet(config, filteredSet, options = {}) {
  const mapType = config.mapType || state.mapType;
  const { xCol, yCol, label } = getMapColumns(mapType);
...
function makeSummaryWorksheet() {
  ...
  const info = [
    ...
    ['Report Includes', 'Strip Map + Wafer Map'],
    ['Current View Map Type', state.mapType],
    ['Current View Group', state.groupValue],
...
function makeStatisticsWorksheet(rows, mapType = state.mapType) {
  const statRows = rowsForMapType(rows, mapType);
  const summary = summarizeRows(statRows);
  const groups = buildStatsGroups(statRows, mapType);
  const aoa = [
...
function getExportGroupValue(mapType) {
  // MERGE/ALL은 Strip, Wafer 양쪽에 공통 적용합니다.
  // 특정 Strip ID 또는 Wafer ID를 선택한 상태라면 선택된 mapType에는 그대로 적용하고,
  // 반대쪽 mapType은 동일 ID 체계가 아니므로 MERGE로 포함해 1개 report 안에 항상 같이 담습니다.
  if (state.groupValue === 'MERGE' || state.groupValue === 'ALL') return state.groupValue;
  return state.mapType === mapType ? state.groupValue : 'MERGE';
}

function appendMapSheetsToWorkbook(wb, used, mapType, filteredSet) {
  const { label } = getMapColumns(mapType);
  const exportGroupValue = getExportGroupValue(mapType);
  const mapConfigs = getDisplayedMapConfigs(mapType, exportGroupValue);
  ...
  mapConfigs.forEach((config, idx) => {
    ...
    if (config.isMerge && String(config.titleGroup).toUpperCase() === 'MERGE') {
      ...
      XLSX.utils.book_append_sheet(wb, buildMapWorksheet(config, filteredSet, { customDisplay: true }), customSheetName);
    }
  });
}
...
function exportExcelReport() {
  try {
    ...
    XLSX_LIB.utils.book_append_sheet(wb, makeStatisticsWorksheet(filteredRows, 'strip'), uniqueSheetName('Strip_Statistics', used));
    XLSX_LIB.utils.book_append_sheet(wb, makeStatisticsWorksheet(filteredRows, 'wafer'), uniqueSheetName('Wafer_Statistics', used));
    appendMapSheetsToWorkbook(wb, used, 'strip', filteredSet);
    appendMapSheetsToWorkbook(wb, used, 'wafer', filteredSet);
