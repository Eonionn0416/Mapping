...
const APP_VERSION = "v17-include-no-cpk-measurement-rows";
...
let cpkTrendChart = null;
let assyTrendChart = null;
let trendRows = [];
let assyTrendRows = [];
let trendStartMonth = "";
...
const el = {
  ...
  nativePickerBtn: document.getElementById("nativePickerBtn"),
  folderPickerBtn: document.getElementById("folderPickerBtn"),
  localBridgeBtn: document.getElementById("localBridgeBtn"),
  ...
  cpkTrendChart: document.getElementById("cpkTrendChart"),
  ...
  assyProcessSelect: document.getElementById("assyProcessSelect"),
  assyCharacteristicSelect: document.getElementById("assyCharacteristicSelect"),
  assyTrendChart: document.getElementById("assyTrendChart"),
...
function setBusy(isBusy) {
  ...
  el.exportBtn.disabled = isBusy || !rawRows.length;
}
...
async function handleExcelEntries(entries, sourceLabel = "File API") {
  ...
  try {
    ...
    else if (readFailed) log("읽힌 row가 없습니다. Drag&Drop/Input File API가 막히면 Permission Picker 또는 Local Bridge Import를 사용하세요.");
  } finally {
...
function fileHandleToEntry(handle) {
  return {
    name: handle.name,
    type: "FileSystemAccessHandle",
    readRows: async () => {
      const allowed = await requestHandleReadPermission(handle);
      if (!allowed) throw new Error(`${handle.name}: 브라우저 파일 읽기 권한이 거부되었습니다.`);
      const freshFile = await handle.getFile();
      return await readExcelFile(freshFile);
    }
  };
}

async function pickWithNativeFilePicker() {
  if (!window.showOpenFilePicker) {
    log("이 브라우저는 Permission Picker(showOpenFilePicker)를 지원하지 않습니다. Chrome 또는 Edge 최신 버전에서 사용하세요.");
    return;
  }
  try {
    ...
    await handleExcelEntries(handles.map(fileHandleToEntry), "Permission Picker");
  } catch (error) {
    if (error && error.name === "AbortError") return;
    console.error(error);
    log(`Permission Picker Error: ${error.message || error}`);
  }
}

async function pickFolderWithNativePicker() {
  if (!window.showDirectoryPicker) {
    log("이 브라우저는 Folder Picker(showDirectoryPicker)를 지원하지 않습니다. Chrome 또는 Edge 최신 버전에서 사용하세요.");
    return;
  }
  try {
    ...
    for await (const [, handle] of directoryHandle.entries()) {
      if (handle.kind === "file" && /\.xls[xm]?$/i.test(handle.name)) entries.push(fileHandleToEntry(handle));
    }
    if (!entries.length) {
      log("선택한 폴더에 .xlsx/.xls 파일이 없습니다.");
      return;
    }
    await handleExcelEntries(entries, "Folder Picker");
  } catch (error) {
    if (error && error.name === "AbortError") return;
    console.error(error);
    log(`Folder Picker Error: ${error.message || error}`);
  }
}

async function importFromLocalBridge() {
  const baseUrl = "http://127.0.0.1:8787";
  try {
    const healthResponse = await fetch(`${baseUrl}/health`, { cache: "no-store" });
    if (!healthResponse.ok) throw new Error(`bridge health ${healthResponse.status}`);
    const fileListResponse = await fetch(`${baseUrl}/files`, { cache: "no-store" });
    if (!fileListResponse.ok) throw new Error(`bridge files ${fileListResponse.status}`);
    const payload = await fileListResponse.json();
    const files = Array.isArray(payload.files) ? payload.files : [];
    const entries = files.filter(file => file && /\.xls[xm]?$/i.test(file.name)).map(file => ({
      name: file.name,
      size: file.size || 0,
      lastModified: file.mtimeMs || 0,
      type: "LocalBridge",
      readRows: async () => {
        const response = await fetch(`${baseUrl}/file?name=${encodeURIComponent(file.name)}`, { cache: "no-store" });
        if (!response.ok) throw new Error(`${file.name}: Local Bridge file fetch 실패 (${response.status})`);
        const buffer = await response.arrayBuffer();
        if (!isReadableZipBuffer(buffer)) throw new Error(`${file.name}: Local Bridge에서 받은 파일이 XLSX/ZIP 형식이 아닙니다.`);
        return await readExcelBuffer(file.name, buffer);
      }
    }));
    if (!entries.length) {
      log("Local Bridge local-upload 폴더에 Excel 파일이 없습니다.");
      return;
    }
    await handleExcelEntries(entries, "Local Bridge");
  } catch (error) {
    console.error(error);
    log(`Local Bridge Error: ${error.message || error} / 사용법: 프로젝트 폴더에서 node local-file-bridge.js 실행 후 local-upload 폴더에 Excel을 넣고 다시 클릭`);
  }
}
...
function refreshBumpFilters() {
  ...
  const prevItem = el.itemSelect.value;
  ...
  const items = uniqueSorted(productRows.map(row => row.item));
  setSelectOptions(el.itemSelect, items, prevItem, false);
  const selectedItem = el.itemSelect.value;
  const itemRows = productRows.filter(row => row.item === selectedItem);
  const dataTypes = uniqueSorted(itemRows.map(row => row.dataType || "Blank"));
...
function refreshAssyFilters() {
  ...
  const prevProcess = el.assyProcessSelect.value;
  const prevCharacteristic = el.assyCharacteristicSelect.value;
  const devices = uniqueSorted(assyRows().map(row => row.device));
  setSelectOptions(el.assyDeviceSelect, devices, prevDevice, false);
  const selectedDevice = el.assyDeviceSelect.value;
  const deviceRows = assyRows().filter(row => row.device === selectedDevice);
  const processes = uniqueSorted(deviceRows.map(row => row.process));
  setSelectOptions(el.assyProcessSelect, processes, prevProcess, false);
  const selectedProcess = el.assyProcessSelect.value;
  const processRows = deviceRows.filter(row => row.process === selectedProcess);
  const characteristics = uniqueSorted(processRows.map(row => row.characteristics));
  setSelectOptions(el.assyCharacteristicSelect, characteristics, prevCharacteristic, false);
}
...
function getFilteredAssyRows() {
  ...
  const process = el.assyProcessSelect.value;
  const characteristics = el.assyCharacteristicSelect.value;
  return assyRows().filter(row => {
    ...
    if (characteristics && row.characteristics !== characteristics) return false;
    return true;
  }).sort(sortByMonth);
}
...
function buildTrendRows(rows, mode) {
  const map = new Map();
  rows.forEach(row => {
    const key = row.reportMonth;
    if (!key) return;
    if (!map.has(key)) {
      map.set(key, {
        reportMonth: key,
        reportLabel: monthLabel(key),
        product: row.product,
        item: row.item,
        dataType: mode === "BUMP" ? (el.dataTypeSelect.value === "__ALL__" ? "All" : (row.dataType || "Blank")) : row.process,
        device: row.device,
        process: row.process,
    ...
    if (normalizeNumber(row.min) !== null) item.minValues.push(normalizeNumber(row.min));
    if (normalizeNumber(row.max) !== null) item.maxValues.push(normalizeNumber(row.max));
    if (normalizeNumber(row.avg) !== null) item.avgValues.push(normalizeNumber(row.avg));
    if (normalizeNumber(row.std) !== null) item.stdValues.push(normalizeNumber(row.std));
    if (normalizeNumber(row.cpk) !== null) item.cpkValues.push(normalizeNumber(row.cpk));
    if (normalizeNumber(row.ppk) !== null) item.ppkValues.push(normalizeNumber(row.ppk));
  });
...
function renderBumpTrendChart() {
  const labels = trendRows.map(row => row.reportLabel || monthLabel(row.reportMonth));
  const title = `${el.productSelect.value || "Product"} / ${el.itemSelect.value || "Item"}`;
  if (cpkTrendChart) cpkTrendChart.destroy();
  cpkTrendChart = new Chart(el.cpkTrendChart, {
    type: "line",
    data: { labels, datasets: trendDatasets(trendRows) },
    options: chartOptions(title, "CPK / PPK")
  });
}

function renderAssyTrendChart() {
  const labels = assyTrendRows.map(row => row.reportLabel || monthLabel(row.reportMonth));
  const title = `${el.assyDeviceSelect.value || "Device"} / ${el.assyProcessSelect.value || "Process"} / ${el.assyCharacteristicSelect.value || "Characteristics"}`;
  if (assyTrendChart) assyTrendChart.destroy();
  assyTrendChart = new Chart(el.assyTrendChart, {
    type: "line",
    data: { labels, datasets: trendDatasets(assyTrendRows) },
    options: chartOptions(title, "CPK / PPK")
  });
}
...
function setupEvents() {
  ...
  if (el.nativePickerBtn) el.nativePickerBtn.addEventListener("click", pickWithNativeFilePicker);
  if (el.folderPickerBtn) el.folderPickerBtn.addEventListener("click", pickFolderWithNativePicker);
  if (el.localBridgeBtn) el.localBridgeBtn.addEventListener("click", importFromLocalBridge);
  ...
  el.uploadBtn.addEventListener("click", uploadSelectedRows);
  el.refreshBtn.addEventListener("click", loadFirestoreData);
  el.clearSelectionBtn.addEventListener("click", () => {
    selectedFiles = [];
    selectedRows = [];
    fileReadStatus = new Map();
    el.excelFiles.value = "";
    renderSelectedFileList();
    updateMetrics({ selected: 0, inserted: 0, skipped: 0 });
    setBusy(false);
  ...
  el.itemSelect.addEventListener("change", () => { refreshBumpFilters(); renderTrend(); });
  ...
  el.assyProcessSelect.addEventListener("change", () => { refreshAssyFilters(); renderTrend(); });
  el.assyCharacteristicSelect.addEventListener("change", renderTrend);
