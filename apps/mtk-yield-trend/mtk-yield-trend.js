...
function setBusy(isBusy) {
  ...
  el.exportAssyBtn.disabled = isBusy || !assyRows.length;
  el.exportOsBtn.disabled = isBusy || !osRows.length;
  el.exportBinBtn.disabled = isBusy || !binRows.length;
}
...
function rebuildDerivedData() {
  ...
  const windowAssyRows = assyMergedRows.filter(row => isCompactDateInTrendWindow(row.sod));
  const windowOsRows = osRows.filter(row => isCompactDateInTrendWindow(row.inputDate));
    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          type: "line",
          label: "LSL 98.5%",
          data: sodSummaryRows.map(() => ASSY_YIELD_LSL),
          yAxisID: "rateAxis",
          borderDash: [6, 5],
          pointRadius: 0,
          borderWidth: 1.5
    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          type: "line",
          label: "USL 0.3%",
          data: osTrendRows.map(() => OS_RATE_USL),
          yAxisID: "rateAxis",
          borderDash: [6, 5],
          pointRadius: 0,
          borderWidth: 1.5
        ...
        },
        {
          type: "bar",
          label: "OS_SS",
          data: osTrendRows.map(row => row.osSs),
          yAxisID: "qtyAxis"
        }
      ]
    },
