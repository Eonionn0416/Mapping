...
v28 Changes (기존 저장된 lot이 새 FT_IN_TIME으로 안 채워지던 문제 수정)
-----------
v27 배포 전에 이미 Firestore에 저장돼 있던 BIN lot을, v27 이후 FT_IN_TIME이 포함된 파일로 다시 업로드해도 **완전한 중복(같은 Report Week + Lot 정보 + 수량)으로 판단되어 그냥 Skip**되고, 기존에 저장된(그리고 FT_IN_TIME이 없는) 문서가 그대로 남아있던 문제를 수정했습니다. 그 결과 해당 lot은 계속 예전 방식(파일명 -7일)으로만 WW가 계산되어, 실제로는 다른 WW(예: WW28)에 반영돼야 할 데이터가 누락된 것처럼 보였습니다.
